...
public class CustomerContactDAO {
  ...
  public void addCustomerContact(CustomerContact contact) {
    String sql =
        "INSERT INTO customer_contact (contactId, customerEmail, phoneOne, phoneTwo,"
            + " preferredContact, alternateContact) VALUES (?, ?, ?, ?, ?, ?)";
  ...
  public void updateCustomerContact(CustomerContact contact) {
    String sql =
        "UPDATE customer_contact SET customerEmail = ?, phoneOne = ?, phoneTwo = ?,"
            + " preferredContact = ?, alternateContact = ? WHERE contactId = ?";
    DatabaseConnection connection = new DatabaseConnection();
    try (Connection conn = connection.connect();
        PreparedStatement pstmt = conn.prepareStatement(sql)) {
